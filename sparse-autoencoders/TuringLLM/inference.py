...
class TuringLLMForInference:
    ...
    def __init__(self, model_path=model_path, collect_activations=False):
        ...
        while xgen.size(1) < max_length:
            with torch.no_grad():
                ...
                if self.model.collect_activations is True:
                    for layer_index in range(len(acts)):
                        if activations[layer_index] is False:
                            activations[layer_index] = acts[layer_index]
                        else:
                            activations[layer_index] = torch.cat((activations[layer_index], acts[layer_index]), dim=0)
