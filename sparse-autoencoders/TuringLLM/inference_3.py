...
from TuringLLM.model_3 import TuringLLM
...
class TuringLLMForInference:
    ...
    def __init__(self, model_path=model_path, collect_activations=False, return_logits=False, sae_models={}, replace_latents_layer_index=0):
        ...
        self.load_model(model_path, sae_models, replace_latents_layer_index)



    def load_model(self, checkpoint_path, sae_models, replace_latents_layer_index):
        ...
        self.model = TuringLLM(checkpoint['config'], sae_models, replace_latents_layer_index)
        self.model.load_state_dict(checkpoint['model'])
        self.model.collect_activations = self.collect_activations
    ...
    def generate_batch(self, batch, max_length=64, tokenize=True, decode=True, ignore_end=False, topk=12):
        ...
        xgen = tokens.to(self.device)
        completed = []
        activations = [False for _ in range(self.model.config.n_layer)]
        
        while xgen.size(1) < max_length:
            with torch.no_grad():
                logits, acts = self.model(xgen)
                if self.model.collect_activations is True and xgen.size(1) >= max_length - 1:
                    activations = acts
        ...
        if self.return_logits is True:
            return results, logits
        
        if self.model.collect_activations is True:
