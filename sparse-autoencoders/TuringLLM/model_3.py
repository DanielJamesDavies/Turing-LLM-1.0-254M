...
class TuringLLM(nn.Module):
    def __init__(self, config, sae_models, replace_latents_layer_index):
        super().__init__()
        self.config = config
        ...
        self.replace_latents_layer_index = replace_latents_layer_index
    ...
    def forward(self, idx, targets=None, input_pos=None):
        ...
        residual_streams = []
        mlp_down_activations = []
        
        for block_index, block in enumerate(self.transformer.h):
            if block_index == self.replace_latents_layer_index:
                x, mlp_down_x = block(x, input_pos, self.sae_models[str(block_index)])
            else:
                x, mlp_down_x = block(x, input_pos)
            if self.collect_activations is True:
                residual_streams.append(x) # [:, -1, :]
                mlp_down_activations.append(mlp_down_x) # [:, -1, :]
        ...
        if self.collect_activations is True:
            with torch.no_grad():
                residual_streams = (act.detach().cpu() for act in residual_streams)
                mlp_down_activations = (act.detach().cpu() for act in mlp_down_activations)
            return logits, [residual_streams, mlp_down_activations]
